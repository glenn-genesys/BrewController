...
class ConfigMenu : public MenuList {
public:
  ConfigMenu(const char *n,
    			  const char * vs[],
    			  int len,
    			  int defaultIndex,
  ...
  Menu *left() {
  	if (!selected) {
  	    ...
  	    switch (getIndex()) {
  	      case 0: { // Lager mode
  	        startPoint->setValue(15.0);
  	        endPoint->setValue(19.0);
  	        duration->setValue(14);
  	        break;
  	      }
  	      case 1: { // Cider mode
  	        startPoint->setValue(18.0);
  	        endPoint->setValue(22.0);
  	        duration->setValue(12);
  	        break;
  	      }
  	      case 2: break;  // Manual mode
  	    };
  		return this;
  	}
  }
...
void menuSetup() {
	...
	contMode = new ConfigMenu("Preset", controlMode, 3, 1);
...
// This would be better if it allowed keypad interrupt -- so check for keypress during delay
// Or if we simply used the keys to step through the values, like MenuSchedule, but showing two arrays and not allowing editing.
void arrayReplay( float data1[], float data2[], int len, int del ) {
  ...
  for (int i=0; i<len; i++) {
    ...
    // Want to check for keyboard interrupt during display
    // so break up delay into 50ms units
    for (int d=0; d<del; d = d + 50) {
      delay(50);
      
      keypad.read();
      
      if (keypad.buttonWasJustPressed()) return;
    }
  }
}

void hourlyReview(Menu &m) {
  arrayReplay( hourlyMin, hourlyMax, 24, 1000 );
}


void dailyReview(Menu &m) {
  arrayReplay( dailyMin, dailyMax, 24, 1000 );
}
...
void runningMode(float setPoint) {
  ...
  } while (!keypad.buttonWasJustPressed());
}
...
void loop(void) {
  ...
  Serial.println(keypad.buttonWasJustPressed(), DEC);
